...
const WebsiteAbout = () => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('websiteAbout');
    if (saved) {
  ...
  const handleSave = () => {
    setIsSaving(true);
    setTimeout(() => {
      localStorage.setItem('websiteAbout', JSON.stringify(formData));
      setIsSaving(false);
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setFormData({ ...formData, heroBanner: reader.result });
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-4">
              {formData.heroBanner && (
                <div className="w-48 h-32 bg-gray-100 rounded border overflow-hidden">
                  <img src={formData.heroBanner} alt="Banner" className="w-full h-full object-cover" />
                </div>
              )}
          ...
          <div>
            ...
            <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full border rounded-lg px-3 py-2" rows="5" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Mission</label>
            <textarea value={formData.mission} onChange={(e) => setFormData({ ...formData, mission: e.target.value })} className="w-full border rounded-lg px-3 py-2" rows="3" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Vision</label>
            <textarea value={formData.vision} onChange={(e) => setFormData({ ...formData, vision: e.target.value })} className="w-full border rounded-lg px-3 py-2" rows="3" />
          </div>
        </div>
      </div>
    </div>
